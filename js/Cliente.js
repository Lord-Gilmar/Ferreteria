$(document).ready(function() {
    ...
    $('#form-crear').submit(e => {
        ...
        console.log(ci)
        funcion = 'crear';
        $.post('../controlador/ClienteController.php', { nombre, apellido, ci, edad, telefono, correo, sexo, adicional, funcion }, (response) => {
            if (response == 'add') {
                $('#add-cli').hide('slow');
                $('#add-cli').show(1000);
                $('#add-cli').hide(2000);
            ...
            if (result.value) {
                $.post('../controlador/ClienteController.php', { id, funcion }, (response) => {
                    ...
                    if (response == 'borrado') {
                        ...
                        swalWithBootstrapButtons.fire(
                            'No se pudo borrar!',
                            'El cliente ' + nombre + ' no fue borrado porque esta siendo usado en un inventario.',
                            'error'
                        )
                    }
                })
