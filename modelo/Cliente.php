...
class Cliente{
    ...
    function crear($nombre,$apellido,$ci,$edad,$telefono,$correo,$sexo,$adicional,$avatar){
        ...
        if(!empty($this->objetos)){
            ...
            $sql="INSERT INTO cliente(nombre,apellidos,ci,edad,telefono,correo,sexo,adicional,avatar) values (:nombre,:apellido,:ci,:edad,:telefono,:correo,:sexo,:adicional,:avatar);";
            $query = $this->acceso->prepare($sql);
            $query->execute(array(':nombre'=>$nombre,':apellido'=>$apellido,':ci'=>$ci,':edad'=>$edad,':telefono'=>$telefono,':correo'=>$correo,':sexo'=>$sexo,':adicional'=>$adicional,':avatar'=>$avatar));
            echo 'add';
        }
    }
